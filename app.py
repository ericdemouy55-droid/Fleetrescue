...
def init_data():
    if not DEPANNEURS_FILE.exists():
        pd.DataFrame([
            ["D001", "BestDrive Roissy", "BestDrive", "Roissy", 49.0040, 2.5170, "+33600000001", "roissy@demo.fr", 90, True, True, True, True, "315/80 R22.5;315/70 R22.5;385/65 R22.5", 4.7],
            ["D002", "Eurotyre Senlis", "Eurotyre", "Senlis", 49.2060, 2.5860, "+33600000002", "senlis@demo.fr", 80, True, False, True, True, "315/80 R22.5;385/65 R22.5", 4.4],
            ["D003", "BestDrive Compiègne", "BestDrive", "Compiègne", 49.4140, 2.8230, "+33600000003", "compiegne@demo.fr", 100, True, True, True, True, "315/80 R22.5;315/70 R22.5;295/80 R22.5", 4.8],
            ["D004", "FleetPartner Amiens", "FleetPartner", "Amiens", 49.8940, 2.2950, "+33600000004", "amiens@demo.fr", 120, True, True, True, True, "315/80 R22.5;385/65 R22.5;315/60 R22.5", 4.2],
            ["D005", "Partenaire Paris Nord", "Alliance", "Paris Nord", 48.9360, 2.3570, "+33600000005", "parisnord@demo.fr", 70, True, True, True, False, "315/70 R22.5;385/55 R22.5", 4.1],
        ], columns=["id","nom","reseau","ville","latitude","longitude","telephone","email","zone_km","route","autoroute","pl","disponible","stock","score"]).to_csv(DEPANNEURS_FILE, index=False)

    if not DEMANDES_FILE.exists():
        pd.DataFrame(columns=["id","date_creation","client","chauffeur","telephone_chauffeur","immatriculation","latitude","longitude","type_panne","lieu","dimension","statut","depanneur_assigne","distance_km","mode_paiement","commentaire","date_cloture"]).to_csv(DEMANDES_FILE, index=False)

    if not TENTATIVES_FILE.exists():
        pd.DataFrame(columns=["id","demande_id","rang","depanneur_id","depanneur_nom","distance_km","canal","statut","date_tentative"]).to_csv(TENTATIVES_FILE, index=False)
...
def trouver_depanneurs(demande, depanneurs):
    ...
    return pd.DataFrame(sorted(rows, key=lambda x: (x["distance_km"], -float(x["score"]))))
...
def accepter_tentative(demande_id):
    ...
    idxs = tentatives[(tentatives.demande_id == demande_id) & (tentatives.statut == "En attente")].index
    if len(idxs) == 0:
        return
    idx = idxs[0]
...
st.title("FleetPartner Rescue 24/7")
st.caption("Prototype démo — dispatch digital de dépannage PL route / autoroute")

tab1, tab2, tab3, tab4 = st.tabs(["🚨 Demande chauffeur", "🧭 Superviseur", "🛠️ Dépanneurs", "📊 Reporting"])
...
with tab1:
    ...
    if st.button("🚨 Demander un dépannage", type="primary"):
        ...
        if candidats.empty:
            ...
            st.dataframe(candidats[["nom","reseau","ville","distance_km","telephone","stock","score"]], use_container_width=True, hide_index=True)
...
with tab2:
    ...
    if demandes.empty:
        ...
        st.dataframe(demandes.sort_values("date_creation", ascending=False), use_container_width=True, hide_index=True)
        selected_id = st.selectbox("Sélectionner une demande", demandes["id"].tolist())
        ...
        if len(active):
            row = active.iloc[0]
            st.warning(f"Sollicitation en cours : {row.depanneur_nom} — {row.distance_km} km — {row.canal}")
